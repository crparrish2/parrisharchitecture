...
simpleGallery.routines={
	...
	addcaptionpanel:function(setting){
		...
		var captionFontSize = Math.floor(slideHeight/30);
		...
		$gallerycaptiontext.css({top:0,marginTop:1,marginBottom:2});
		...
		if(setting.showGallery){
		    ...
		    if(captionPanelHt == 0){
		       captionPanelHt = setting.calcCaptionPanelHeight;
		       if(captionPanelHt == 0){
		          captionPanelHt = 25;
		       }
		    }
		    ...
		    topPos = slideHeight - 30 - captionPanelHt;
		}
		...
		setting.$captionpanel.css({top:topPos, height:setting.captionpanelheight}).find('div').css({height:'100%'})
